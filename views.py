...
def _query_block(
    ...
    if query:
        bases = ["doc_title", "title", "content"] if is_section else ["title", "summary"]
        fields: List[str] = []
        for base in bases:
            if lang:
                fields.append(f"{base}.{lang}^{3}" if base == 'title' else f"{base}.{lang}")
            else:
                fields.extend([
...
def _highlight_block(query_present: bool, is_section: bool = False) -> Dict[str, Any]:
    ...
    fields = {
        "title.*": {"fragment_size": SNIPPET_LENGTH // 2, "number_of_fragments": 2},
        ("content.*" if is_section else "summary.*"): {
            "fragment_size": SNIPPET_LENGTH // 2,
...
def _format_hit(
    ...
    if is_section:
        ...
        if src.get("path") and language:
            href = f"/docs/{src.get('path')}/{language}/index.html"
        elif src.get("path"):
            href = f"/docs/{src.get('path')}/index.html"
        else:
            href = "#"
...
def api_search(request: HttpRequest) -> JsonResponse:
    ...
    results: List[Dict[str, Any]] = []
    for hit in raw_hits:
        fmt = _format_hit(hit, lang, query, query_present, is_section)
        results.append(fmt)
